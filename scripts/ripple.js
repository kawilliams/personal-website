...
var cHeight = 100; //Canvas Height
var waterLevel = 50; //The y value of the surface of the water
...
$(document).ready(function () {
    $('footer, #ripple').hover(function (e) {
        if (canvas.getContext) {
            if (!animationActive) {
                ...
                counter = 0;
                points = [];
                points[0] = [spreadSpeed, 0];//The outer points ground out the mid point
...
function setCanvas() {
    if (canvas.getContext) {//Check that canvas element will fire
        ...
        if (ripple.attr('class') === 'ripple_adjust_width') viewportWidth += 15;
        ripple.attr('width', viewportWidth);
        ripple.attr('height', cHeight);
...
function drawShape() {
    ...
    if (counter <= dur) {
        points[midPointIndex] = [0, midPointY];
    ...
    for (var pt = 0; pt < totalPoints; pt++) {
        if (points[pt]) {
            if (pt < midPointIndex) {
                ...
                points[(midPointIndex - pt) + midPointIndex] = [-points[pt][0], points[pt][1]]; //Create an opposite point
            }
